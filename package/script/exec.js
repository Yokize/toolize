...
const execCallback = (notify, callback) => (code, stdout, stderr) => {
  ...
  if (notify && code === 0) notify.succeed(green(notify.text));
  else notify.fail(red(notify.text));
...
// Create a path to the file based on the provided directory and file
// name with additional verification of its existence.
const filePath = (dir, file) => {
  // Resolve the path using the provided or defaulted directory.
...
// installed in the local `node_modules/.bin` or global directory.
...
exports.throwError = (message) => {
  // Print the colorful error to stdout.
  echo(red(message));

  // Exit process  with a non-zero code.
  process.exit(1);
};
